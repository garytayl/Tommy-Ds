...
type ScheduleTodayStripProps = {
  todayDateKey: string;
  jobs: ScheduleJob[];
};

export function ScheduleTodayStrip({ todayDateKey, jobs }: ScheduleTodayStripProps) {
  const date = new Date(todayDateKey + "T12:00:00");
            ...
            jobs.map((job) => {
              ...
              return (
                ...
                >
                  <Link
                    href={`/jobs/${job.id}`}
                    className="block w-full truncate px-2 py-0.5 text-xs font-medium"
